...
Real WB-MR6C/WB-MR6CU/WB-MCM8 smoke tests are opt-in and read-only. They open a direct Modbus RTU serial connection, so no other Modbus master should be using the same RS-485 bus.
...
The real-device smoke tests connect and close the serial transport, read the model and firmware registers, read relay command coils only for relay modules, read input states only for input-capable modules, and read relay-state discrete inputs only when the firmware and model report support for them.
