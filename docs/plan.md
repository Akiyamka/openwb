...
| 10. Mapping matrix configuration UI | `0.10.0` | Add a Home Assistant UI surface for editing local input behavior. The UI should load input modes and matrix data on demand, show inputs `1..6` and `0`, expose supported events and outputs, validate edits before save, and submit the complete desired input modes plus matrix changes through the backend. | Users can configure local WB button-to-relay mappings without editing raw registers. |
