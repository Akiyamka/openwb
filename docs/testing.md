...
The real-device smoke tests connect and close the serial transport, read the model and firmware registers, read relay command coils, read input states, and read relay-state discrete inputs only when the firmware reports support for them.

When reporting results, include the full test command, serial settings, device address, test output, and the printed line that starts with `WB-MR6C real-device smoke result:`. That line includes the decoded model, firmware, relay command states, input states, and relay states if supported by the device firmware.
